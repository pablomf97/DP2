...
public class ActorService {
	...
	public String checkEmail(final String email, final String authority) {
		...
		if (authority.equals("ADMIN") && matcher.matches()) {
			//TODO: faltaría comprobar si se intenta insertar un admin y que compruebe su correo para su caso
			//falta el pattern de admin
			final Pattern patternAdmin = Pattern.compile("(^(([a-z]|[0-9]){1,}[@]{1}([a-z]|[0-9]){1,}([.]{1}([a-z]|[0-9]){1,}){1,})$)|(^((([a-z]|[0-9]){1,}[ ]{1}){1,}<(([a-z]|[0-9]){1,}[@]{1}([a-z]|[0-9]){1,}([.]{1}([a-z]|[0-9]){1,}){1,})>)$)");
